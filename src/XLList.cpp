...
template<typename T>
Node<T> *XLList<T>::Iterator::xorNodes(Node<T> *first, Node<T> *second) const {
...
bool XLList<T>::removeElement(int index) {
    ...
    if (size == 1) {
        ...
        if (index == size - 1) {
            Node<T> *previousNode = Iterator::xorNodes(tail, head);
            ...
            previousToDelete->link = Iterator::xorNodes(previousToDelete->lin, listIterator.nodePtrCurrent);
